...
static uint8_t UUID_SERVICE_MOTOCONSDK_test[16] = { 0x7Au, 0x15u, 0xC0u, 0x28u, 0xA4u, 0x9Du, 0xEAu, 0xA5u, 0xA7u, 0x46u, 0xCFu, 0xECu, 0xF4u, 0xCDu, 0xA2u, 0xAFu };
static uint8_t UUID_SERVICE_MOTOCONSDK[16] = { 0x7Au, 0x15u, 0xC0u, 0x28u, 0xA4u, 0x9Du, 0xEAu, 0xA5u, 0xA7u, 0x46u, 0xCFu, 0xECu, 0xF4u, 0xCDu, 0xA2u, 0xAFu };
/******************************************************
...
void                          hci_control_le_set_con_advertisement_data(void);
void                          hci_control_le_set_non_con_advertisement_data(uint8_t* device_name,uint32_t device_name_length);
static void                   hci_control_le_handle_garmin_advertise_cmd(uint8_t* p_data, uint32_t data_length);
...
static void hci_control_le_set_advertisement_data(const wiced_bt_cfg_settings_t *settings)
{
    ...
    adv_elem[num_elem].len          = strlen( (char *)settings->device_name );
    adv_elem[num_elem].p_data       = ( uint8_t* )settings->device_name;
    num_elem++;

#if 0	//dbg, 060121
    adv_elem[num_elem].advert_type  = BTM_BLE_ADVERT_TYPE_16SRV_COMPLETE;
    adv_elem[num_elem].len          = 2;
    adv_elem[num_elem].p_data[0]    = 0xA0;
    adv_elem[num_elem].p_data[1]    = 0xE0;
    num_elem++;
#endif

    adv_elem[num_elem].advert_type  = BTM_BLE_ADVERT_TYPE_128SRV_COMPLETE;
    adv_elem[num_elem].len          = 16;
    adv_elem[num_elem].p_data       = UUID_SERVICE_MOTOCONSDK_test;
    num_elem++;
...
#ifdef ADV_FOREVER
    WICED_BT_TRACE( "conn_down> adv %d\r\n",
    ...
    for (i = 0; i < app_gatt_db_ext_attr_tbl_size; i++)
    {
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (app_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
                ...
                #else
                    /* Read request send to mcu  */
                    tx_buf[0] = conn_id & 0xff;
                    tx_buf[1] = ( conn_id >> 8 ) & 0xff;
                    tx_buf[2] = attr_handle & 0xff;
                    tx_buf[3] = ( attr_handle >> 8 ) & 0xff;
                    wiced_transport_send_data( HCI_CONTROL_GATT_EVENT_READ_REQUEST, tx_buf, 4 );
...
wiced_bt_gatt_status_t hci_control_le_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    ...
    WICED_BT_TRACE( "%s handle:%d len:%d p_val:%02x %02x\n\r", __FUNCTION__, attr_handle, len, p_val[0], p_val[1] );
    // Check for a matching handle entry
    for (i = 0; i < app_gatt_db_ext_attr_tbl_size; i++)
    {
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
    ...
    if (!is_handle_in_table)
    {
        WICED_BT_TRACE( "Not found handle\n\r" );
        res = WICED_BT_GATT_PENDING;
    }
...
wiced_bt_gatt_status_t hci_control_le_gatt_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data )
{
    ...
    WICED_BT_TRACE("hci_control_le_gatt_callback,Free mem:%d, event = %d \r\n", wiced_memory_get_free_bytes(), event);
    switch( event )
    {
...
/*Add by Garmin define command */
    case HCI_CONTROL_LE_COMMAND_ADVERTISE_GARMIN:
        hci_control_le_handle_garmin_advertise_cmd(p, data_len);
        break;
    default:
        hci_control_send_command_status_evt( HCI_CONTROL_GATT_EVENT_COMMAND_STATUS,
    ...
    else
    {
        ...
        if ( wiced_bt_ble_get_available_tx_buffers( ) > 1 )
        {
            ...
            else
            {
                ...
                hci_control_le_pending_tx_buffer.tx_buf_saved  = WICED_FALSE;
                WICED_BT_TRACE( "Send notification success\n\r" );
...
/*
 * When 89820 initialized, set BLE advertise connectable data
 */
void hci_control_le_set_con_advertisement_data(void)
{
    ...
    adv_elem[num_elem].advert_type  = BTM_BLE_ADVERT_TYPE_128SRV_COMPLETE;
    adv_elem[num_elem].len          = 16;
    adv_elem[num_elem].p_data       = UUID_SERVICE_MOTOCONSDK;
    num_elem++;
...
/*
 * When 89820 initialized, set BLE advertise non-connectable data
 */
void hci_control_le_set_non_con_advertisement_data(uint8_t* device_name,uint32_t device_name_length)
{
    wiced_bt_ble_advert_elem_t adv_elem[2];
...
void hci_control_le_handle_garmin_advertise_cmd(uint8_t* p_data, uint32_t data_length)
{
    wiced_result_t          result = WICED_SUCCESS;
    ble_advertising_type_t  ble_advertising_type = BLE_ADV_OFF;
    wiced_bt_ble_advert_type_t adv_data_type;
    uint8_t adv_data_len = 0;
    //Byte[0]: (1-byte) Advertising mode specified in BLE_advertising_mode_e
    ble_advertising_type = *(p_data);
    //Byte[1]: (1-byte) Data type specified in BLE_advertising_data_type_e
    adv_data_type = *(p_data+1);
    //Byte[2]: (1-byte) Data length
    adv_data_len = *(p_data+2);
    ...
    case BLE_ADV_NON_CONNECTABLE:
        hci_control_le_set_non_con_advertisement_data(p_data + 3, adv_data_len );
    ...
    case BLE_ADV_CONNECTABLE:
        ...
        hci_control_le_set_scan_response_data(p_data + 3, adv_data_len);
...
#endif
int       i;
uint8_t   tx_buf[LE_PROCESS_DATA_MAX_LEN];
uint32_t  remain_length = len;
uint32_t  data_idx = 0;

tx_buf[0] = conn_id & 0xff;
tx_buf[1] = ( conn_id >> 8 ) & 0xff;
tx_buf[2] = handle & 0xff;
tx_buf[3] = ( handle >> 8 ) & 0xff;
tx_buf[4] = len & 0xff;
tx_buf[5] = ( len >> 8 ) & 0xff;

while( remain_length > ( MAX_TRANSPORT_DATA_SIZE - LE_PROCESS_DATA_HEADER_LEN ) )
    {
    tx_buf[6] = data_idx & 0xff;
    tx_buf[7] = ( data_idx >> 8 ) & 0xff;

    memcpy( tx_buf + 8, data + data_idx, MAX_TRANSPORT_DATA_SIZE - LE_PROCESS_DATA_HEADER_LEN );
    wiced_transport_send_data( type, tx_buf, MAX_TRANSPORT_DATA_SIZE );

    data_idx += MAX_TRANSPORT_DATA_SIZE - LE_PROCESS_DATA_HEADER_LEN;
    remain_length -= MAX_TRANSPORT_DATA_SIZE - LE_PROCESS_DATA_HEADER_LEN;
    }
...
if( remain_length > 0 )
    ...
    memcpy( tx_buf + LE_PROCESS_DATA_HEADER_LEN, data + data_idx, remain_length );
    wiced_transport_send_data( type, tx_buf, remain_length + LE_PROCESS_DATA_HEADER_LEN );
    }
}
